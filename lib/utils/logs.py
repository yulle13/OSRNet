...
class _CustomFormatter(logging.Formatter):
    ...
    def __init__(self, fmt, colorful: bool = True):
        self.FORMATS = {
            logging.DEBUG: _CustomFormatter.GREY + fmt + _CustomFormatter.RESET,
            logging.INFO: _CustomFormatter.BLUE + fmt + _CustomFormatter.RESET,
            logging.WARNING: _CustomFormatter.YELLOW + fmt + _CustomFormatter.RESET,
            logging.ERROR: _CustomFormatter.RED + fmt + _CustomFormatter.RESET,
            logging.CRITICAL: _CustomFormatter.BOLD_RED + fmt + _CustomFormatter.RESET,
        }
        self.fmt = fmt
...
class MyLog:
    ...
    def __init__(self, name:str = "my_logger") -> None:
    ...
    def set_file_dir(self, log_file_dir: str = None):
        if log_file_dir:
            log_file_dir = ospath.join(log_file_dir, "%s.log" % time.strftime("%Y-%m-%d_%H-%M-%S", time.localtime()))
            self._fh1 = logging.FileHandler(filename=log_file_dir, mode="w")
            self._fh1.setFormatter(_CustomFormatter(self._fmt, colorful=False))  # file can't show color correctly, disable it
...
if __name__ == "__main__":
    ...
    my_logger.warning("This is  WARNING of my_logger !!")
    my_logger.error("This is  ERROR of my_logger !!")
    my_logger.set_file_dir()  # remove file, so next message should not appear in the log file
    my_logger.critical("This is  CRITICAL of my_logger !!")
